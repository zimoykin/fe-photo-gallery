...
export const apiUpdateFolderById = async (id: string, folder: IFolder) => {
  return apiClient
    .put<IUserFolder>(`/folders/${id}`, folder)
    .then((response) => {
      if (response?.status !== 200) {
        throw new Error('Failed to get folders');
      }
...
export const apiCreateFolder = async (folder: IFolder) => {
  return apiClient
    .post<IUserFolder>('/folders', folder)
    .then((response) => {
      if (response?.status !== 200) {
        throw new Error('Failed to get folders');
      }
...
export const apiFetchPhotoById = async (folderId: string, photoId: string) => {
  return apiClient
    .get<IPhoto>(`/photos/${folderId}/${photoId}/compressed`)
    .then((response) => {
      if (response?.status !== 200) {
...
export const apiFetchUserProfileById = async (id: string) => {
  return apiClient
    .get<IProfile>(`/public/profiles/${id}`)
    .then((response) => {
      if (response?.status !== 200) {
