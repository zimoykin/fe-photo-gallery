...
const FolderView: React.FC = () => {
  ...
  useEffect(() => {
    if (folderId && profileId)
      ApiClient.get<IPublicPhoto[]>(`/public/photos/${profileId}/${folderId}`).then((photos) =>
        SetPhotos(photos)
      );
  }, [folderId, profileId]);

  return (
    <div className="global-background-layer w-100 h-100 scroll p-10">
      <div className="flex-center wrap gap">
        {photos.map(({ photo, profile }) => (
          <div key={photo.id} className="gap">
            <PhotoCard photo={photo} profileImageUrl={profile.url} />
          </div>
        ))}
      </div>
    </div>
