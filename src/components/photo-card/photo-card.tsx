...
const PhotoCard: React.FC<Props> = ({ photo, showEditBtns, needRefresh, removePhotoFromList, profileImageUrl }) => {
    ...
    const uploadFile = useCallback(() => {
        ...
        formData.append('favorite', editInfo.favorite?.toString() ?? 'false');
    ...
    return (
        <>
            <div className={`scale-s photo-card-container flex-center flex-column p-3 shadow ${photo.id ? '' : 'photo-card-new-element'}`}>
                {isLoading ? <CameraSpinner /> :
                    <> <div className="photo-card-img no-repeat radius-5"
                        style={{
                            backgroundImage: `url(${photo.url})`,
                        }}
                    />
                        <div className="photo-card-btns flex-row flex-center w-100">
                            {showEditBtns && <div className="flex-row p-10 w-100 ">
                                {/* left */}
                                <div className="w-50 flex-row align-left ">
                                    <div className="p-3 scale-l hover-bg"
                                        onClick={handleFavoriteClick}
                                    >
                                        <span className="photo-card-description">
                                            {photo.id && <i className="fas fa-star" style={{ fontSize: '1.5rem', color: `${photo.favorite === true ? 'gold' : 'gray'}` }} />}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="w-50 flex-row align-right">
                                    ...
                                    <div className="pl-10 hover-bg p-3"
