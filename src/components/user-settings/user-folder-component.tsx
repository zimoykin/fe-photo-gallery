...
const UserFolder: React.FC<Props> = ({ folder }) => {
  ...
  useEffect(() => {
    if (profile?.id) {
      ApiClient.get<IUserFolder[]>('/folders/' + profile.id)
  ...
  const handleUploadImages = () => {
    if (folder.id) navigate(`/settings/upload/${folder.id}`);
  };
          ...
          {editMode ? (
            <div className="flex-center gap w-100 h-100">
              <input
                ...
                className="w-100 p-2 palitra-6 palitra-color-3 radius-5"
                type="text"
                defaultValue={editedFolder.title}
              />
              <input
                onChange={(e) => setEditedFolder({ ...editedFolder, description: e.target.value })}
                className="w-100 p-2 palitra-6 palitra-color-3 radius-5"
                type="text"
                defaultValue={editedFolder.description}
              />
            </div>
          ) : (
            <span>
              {editedFolder.title} {`(${folder.totalPhotos})`}{' '}
            </span>
          )}
        </div>
        <div className="flex-cente align-right p-10 w-30">
          {!editMode && (
            <i className="scale-l hover-bg p-2 fas fa-image" onClick={handleUploadImages} />
          )}
          {editMode ? (
            <div className="flex-center gap">
              <i className="scale-l hover-bg p-2 fas fa-save" onClick={handleClickSaveFolder} />
              <i
                className="scale-l hover-bg p-2 fas fa-close palitra-1"
                onClick={() => setEditMode(!editMode)}
              />
            </div>
          ) : (
            <i className="scale-l hover-bg p-2 fas fa-pen" onClick={() => setEditMode(!editMode)} />
          )}
          {!editMode && (
            <i
              className="scale-l hover-bg p-2 fas fa-trash"
